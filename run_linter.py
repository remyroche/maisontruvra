...
def run_command(command, description):
    ...
    try:
        ...
        print(
            f"Error: '{command[0]}' command not found. Is it installed and in your PATH?",
            file=sys.stderr,
        )
        sys.exit(1)
        ...
        print(
            f"Error during '{description}'. Return code: {e.returncode}",
            file=sys.stderr,
        )
        ...
        print(
            f"An unexpected error occurred during '{description}': {e}", file=sys.stderr
        )
        sys.exit(1)
...
def main():
    ...
    format_command = ["ruff", "format", ".", f"--exclude={EXCLUDE_DIR}"]
    ...
    try:
        ...
        result = subprocess.run(
            fix_command, capture_output=True, text=True, check=False
        )
        ...
        if result.returncode > 1:
            print(
                f"Ruff encountered an error. Stderr:\n{result.stderr}", file=sys.stderr
            )
        ...
        print(
            "Error: 'ruff' command not found. Is Ruff installed and in your PATH?",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if issues_found:
        ...
        try:
            unfixed_command = base_check_command + [
                f"--output-file={unfixed_report_file}"
            ]
            subprocess.run(unfixed_command, check=False)
            print(f"--- Remaining issues saved to {unfixed_report_file} ---")
        except Exception as e:
            print(
                f"An error occurred while checking for unfixed issues: {e}",
                file=sys.stderr,
            )
    else:
