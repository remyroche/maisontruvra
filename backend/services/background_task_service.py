...
class BackgroundTaskService:
    ...
    def queue_order_processing(order_id: int):
        ...
        try:
            ...
            celery.send_task(
                "send_email",
                args=[{"type": "order_confirmation", "order_id": order_id}],
            )

            MonitoringService.log_info(
                f"Background tasks queued for order {order_id}", "BackgroundTaskService"
            )
    ...
    def queue_user_welcome_email(user_id: int):
        ...
        try:
            celery.send_task(
                "send_email", args=[{"type": "welcome", "user_id": user_id}]
            )

            MonitoringService.log_info(
                f"Welcome email queued for user {user_id}", "BackgroundTaskService"
            )
    ...
    def queue_low_stock_alert():
        ...
        try:
            ...
            MonitoringService.log_info(
                "Low stock check task queued", "BackgroundTaskService"
            )

        except Exception as e:
            MonitoringService.log_error(
                f"Failed to queue low stock check: {str(e)}", "BackgroundTaskService"
            )
