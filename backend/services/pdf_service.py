...
class PDFService:
    ...
    def generate_invoice(self, order):
        ...
        try:
            self.logger.info(
                f"Generating invoice for order {order.id} using Playwright."
            )
            # Determine template based on user type (B2C vs B2B)
            template_name = (
                "non-email/b2b_invoice.html"
    ...
    def generate_passport_pdf(self, passport_data):
        ...
        try:
            ...
            self.logger.info(
                f"Generating passport PDF for product: {product_name} using Playwright."
            )
            ...
            self.logger.error(
                f"Failed to generate passport PDF for '{product_name}': {e}"
            )
            return None
