from backend.models import (
    Product, Category, Collection, User, B2BAccount, 
    BlogPost, Order, Review, AdminAuditLog
)
...
class RecyclingBinService:
    ...
    def get_soft_deleted_items(self):
        ...
        for model_name, model_class in MODEL_MAP.items():
            ...
            deleted_items = model_class.query.filter(model_class.deleted_at.isnot(None)).all()
    ...
    def get_deletion_logs(self, item_type: str, item_id: int):
        ...
        ).order_by(AdminAuditLog.timestamp.desc()).all()
