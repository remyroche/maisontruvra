...
class UnifiedAuthService:
    ...
    def register_user(self, user_data: dict[str, Any]) -> User:
        ...
        user = User(
            ...
            if user_data.get("user_type") == "b2b"
            else UserType.B2C,
    ...
    def authenticate_user(self, email: str, password: str) -> tuple[User, bool]:
        ...
        if user.is_staff or user.is_admin:
            ...
            requires_2fa = user.is_2fa_enabled and (
                user.is_totp_enabled or user.is_magic_link_enabled
            )
    ...
    def _verify_magic_link(self, user: User, token: str) -> bool:
        ...
        if (
            not user.magic_link_token
            or user.magic_link_token != token
            or not user.magic_link_expires_at
            or datetime.utcnow() > user.magic_link_expires_at
        ):
            return False
    ...
    def _send_magic_link_email(self, user: User, token: str):
        ...
        magic_link_url = generate_secure_url('unified_auth.magic_link_login', token=token)
    ...
    def admin_disable_mfa(self, user_id: int) -> bool:
        ...
        EmailService.send_2fa_status_change_email(
            user, enabled=False, admin_triggered=True
        )
