...
class RBACService:
    ...
    def __init__(self):
        ...
        self._role_permissions = {
            "Admin": {
                "MANAGE_USERS", "VIEW_REPORTS", "DELETE_DATA", "MANAGE_PRODUCTS", 
                "VIEW_INVENTORY", "FULL_ACCESS", "ADMIN_ACCESS", "VIEW_USERS",
                "VIEW_B2B_ACCOUNTS", "MANAGE_B2B_ACCOUNTS", "VIEW_PRODUCTS",
                "VIEW_ORDERS", "MANAGE_ORDERS", "MANAGE_BLOG", "MANAGE_SITE_SETTINGS",
                "MANAGE_LOYALTY", "MANAGE_INVOICES", "VIEW_AUDIT_LOGS"
            },
            "Staff": {
                "VIEW_REPORTS", "PROCESS_ORDERS", "VIEW_PRODUCTS", "VIEW_ORDERS",
                "MANAGE_ORDERS"
            },
            "Manager": {
                "MANAGE_PRODUCTS", "VIEW_INVENTORY", "GENERATE_REPORTS", "VIEW_PRODUCTS",
                "VIEW_ORDERS", "MANAGE_ORDERS", "VIEW_REPORTS"
            },
            "Editor": {
                "CREATE_CONTENT", "EDIT_CONTENT", "UPLOAD_MEDIA", "MANAGE_BLOG"
            },
            "Viewer": {
                "VIEW_CONTENT", "DOWNLOAD_REPORTS"
            },
            "B2B": {
                "VIEW_B2B_DASHBOARD", "ACCESS_B2B_DATA", "VIEW_PRODUCTS", "VIEW_ORDERS"
            },
            "Customer": {
                "VIEW_PRODUCTS", "PLACE_ORDERS", "VIEW_OWN_ORDERS"
            }
        }
    ...
    def _get_user_roles_from_db(self, user_id):
        ...
        try:
            ...
            if hasattr(user, 'roles') and user.roles:
                ...
                return {user.role.name if hasattr(user.role, 'name') else str(user.role)}
            else:
                return set()
    ...
    def assign_role(self, user_id, role_name):
        ...
        try:
            user = User.query.get(user_id)
            if not user:
                security_logger.warning(f"Cannot assign role '{role_name}' to non-existent user '{user_id}'.")
                return False
    ...
    def remove_role(self, user_id, role_name):
        ...
        try:
            user = User.query.get(user_id)
            if not user:
                security_logger.warning(f"Cannot remove role from non-existent user '{user_id}'.")
                return False
            ...
            if hasattr(user, 'roles') and role in user.roles:
                ...
                security_logger.debug(f"User '{user_id}' does not have role '{role_name}'.")
                return False
    ...
    def create_role(self, role_name, permissions=None):
        ...
        security_logger.info(f"Created new role '{role_name}' with permissions: {permissions}")
        return True
    ...
    def update_role_permissions(self, role_name, permissions):
        ...
        self._role_permissions[role_name] = set(permissions)
        security_logger.info(f"Updated permissions for role '{role_name}': {permissions}")
        return True
