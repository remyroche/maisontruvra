...
from ..utils.cache_helpers import (
    get_delivery_methods_key,
    clear_delivery_methods_cache
)
...
class DeliveryService:
    ...
    def get_active_methods_cached():
        ...
        if methods is None:
            methods = DeliveryMethod.query.filter_by(is_active=True).order_by(DeliveryMethod.price).all()
    ...
    def get_available_methods_for_user(user_id: int):
        ...
        if not user or not user.loyalty_tier:
            ...
            return [method.to_dict() for method in default_tier.delivery_methods if method.is_active]
        
        return [method.to_dict() for method in user.loyalty_tier.delivery_methods if method.is_active]
    ...
    def delete_method(method_id: int):
        ...
        method = DeliveryMethod.query.get_or_404(method_id)
        method_details = method.to_dict(include_tiers=True) # Get details before deleting
