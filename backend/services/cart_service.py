...
class CartService:
    ...
    def get_cart_details(self, user_id: int) -> dict:
        ...
        if user.user_type == UserType.B2B and user.tier:
            discount_multiplier = Decimal("1") - (
                user.tier.discount_percentage / Decimal("100")
            )
        ...
        for item in cart.items:
            ...
            final_price = (
                Decimal("0.00")
                if item.is_reward
                else (original_price * discount_multiplier).quantize(Decimal("0.01"))
            )
    ...
    def add_item(
        ...
        if product.owner_id and product.owner_id != user_id:
            self.logger.warning(
                f"User {user_id} attempted to add exclusive product {product_id}."
            )
            raise PermissionError(
                "This product is exclusive and cannot be added to your cart."
            )
        ...
        try:
            cart_item = CartItem.query.filter_by(
                cart_id=cart.id, product_id=product_id, is_reward=False
            ).first()
            if cart_item:
            ...
            db.session.commit()
            self.logger.info(
                f"Added {quantity} of product {product_id} to cart for user {user_id}."
            )
            return cart
            ...
            self.logger.error(
                f"Failed to add item to cart for user {user_id}: {e}", exc_info=True
            )
            raise ServiceError("Could not add item to cart.") from e

    def update_item_quantity(
        self, user_id: int, item_id: int, new_quantity: int
    ) -> Cart:
        ...
        try:
            if quantity_diff > 0:
                InventoryService.reserve_stock(
                    cart_item.product_id, quantity_diff, user_id
                )
            ...
            db.session.commit()
            self.logger.info(
                f"Updated cart item {item_id} to quantity {new_quantity} for user {user_id}."
            )
            return cart_item.cart
        except Exception as e:
            db.session.rollback()
            self.logger.error(
                f"Failed to update cart item {item_id}: {e}", exc_info=True
            )
    ...
    def remove_item(self, user_id: int, item_id: int) -> Cart:
        ...
        try:
            ...
            db.session.rollback()
            self.logger.error(
                f"Failed to remove cart item {item_id}: {e}", exc_info=True
            )
    ...
    def clear_cart(self, user_id: int) -> bool:
        ...
        try:
            ...
            db.session.rollback()
            self.logger.error(
                f"Failed to clear cart for user {user_id}: {e}", exc_info=True
            )
    ...
    def add_reward_to_cart(self, user_id: int, reward_id: int) -> Cart:
        ...
        if not reward or not reward.linked_product_id:
            raise ValidationException(
                "Reward is not a product and cannot be added to the cart."
            )
        ...
        try:
            ...
            db.session.rollback()
            self.logger.warning(
                f"Attempted to add duplicate reward {reward_id} to cart for user {user_id}."
            )
