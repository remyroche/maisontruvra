...
class CartService:
    ...
    def get_cart_details(self, user_id):
        ...
        if user.user_type == UserType.B2B and user.tier:
            ...
            discount_multiplier = Decimal("1") - (
                tier.discount_percentage / Decimal("100")
            )

        for item in cart.items:
            original_price = item.product.price
            # Apply tier discount if applicable
            final_price = (original_price * discount_multiplier).quantize(
                Decimal("0.01")
            )
    ...
    def add_reward_to_cart(user_id, reward_id):
        ...
        if not reward or not reward.linked_product_id:
            raise ValidationException(
                "This reward is not a physical product and cannot be added to the cart."
            )

        # Check if the reward is already in the cart to prevent duplicates
        existing_item = CartItem.query.filter_by(
            cart_id=cart.id, product_id=reward.linked_product_id, is_reward=True
        ).first()
        if existing_item:
    ...
    def add_item_to_cart(self, user_id, product_id, quantity, custom_price=None):
        ...
        if product.owner_id and product.owner_id != user_id:
            self.logger.warning(
                f"User {user_id} tried to add exclusive product {product_id} owned by {product.owner_id}."
            )
        ...
        db.session.commit()
        self.logger.info(
            f"Added/updated product {product_id} in cart for user {user_id}."
        )
        return cart
    ...
    def remove_from_cart(user_id: int, product_id: int) -> Cart:
        item_id = "N/A"
        quantity_to_release = (
            0  # Default to 0 to prevent errors if cart_item is not found
        )
        try:
            cart = Cart.query.filter_by(user_id=user_id).first()
            if cart:
                ...
                if cart_item:
                    ...
                    InventoryService.release_stock(
                        product_id, quantity_to_release, user_id=user_id
                    )

                    MonitoringService.log_info(
                        f"Cart item removed: Item {item_id}", "CartService"
                    )
            return cart
        except Exception as e:
            db.session.rollback()
