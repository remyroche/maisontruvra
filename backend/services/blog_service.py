...
class BlogService:
    ...
    def _sanitize_article_data(self, article_data: dict) -> dict:
        ...
        for field in [
            ...
            if field in article_data:
                ...
                sanitized_data[field] = (
                    InputSanitizer.sanitize_string(article_data[field])
                    if article_data[field] is not None
                    else None
                )
        ...
        if "content" in article_data:
            ...
            sanitized_data["content"] = (
                None  # Ensure content is explicitly set to None if not provided
            )
    ...
    def create_article(self, article_data: dict) -> BlogPost:
        ...
        sanitized_content = InputSanitizer.sanitize_html(
            article_data["content"], allow_tags=True
        )
    ...
    def get_all_articles(self):
        ...
        if posts is None:
            posts = (
                self.db.session.query(BlogPost)
    ...
    def update_article(self, article_id: int, article_data: dict) -> BlogPost:
        ...
        if "content" in article_data:
            post.content = InputSanitizer.sanitize_html(
                article_data["content"], allow_tags=True
            )
    ...
    def create_category(self, category_data: dict) -> BlogCategory:
        ...
        if self.db.session.query(BlogCategory).filter_by(name=sanitized_name).first():
            raise ValidationException(
                f"Category with name '{sanitized_name}' already exists."
            )
    ...
    def update_category(self, category_id: int, category_data: dict) -> BlogCategory:
        ...
        if (
            sanitized_name != category.name
            and self.db.session.query(BlogCategory)
            .filter_by(name=sanitized_name)
            .first()
        ):
            raise ValidationException(
                f"Category with name '{sanitized_name}' already exists."
    ...
    def delete_category(self, category_id: int):
        ...
        if category.blog_posts and len(category.blog_posts) > 0:
            raise ValidationException(
                f"Cannot delete category '{category.name}' because it has associated blog posts."
            )
