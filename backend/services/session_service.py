...
class SessionService:
    ...
    def get_all_active_sessions(page=1, per_page=20):
        ...
        sessions_page = PersistentSession.query.join(User).filter(
            PersistentSession.is_active == True
        ).order_by(
            PersistentSession.last_seen.desc()
    ...
    def terminate_session(session_id: str, performing_user_id: int):
        ...
        """
        current_app.logger.info(f"Admin {performing_user_id} is attempting to terminate session {session_id}.")
        
        session_to_terminate = PersistentSession.query.get(session_id)
        
        if not session_to_terminate:
            current_app.logger.warning(f"Attempted to terminate non-existent session ID: {session_id}")
            return False
        ...
        current_app.logger.info(f"Successfully terminated session {session_id} by admin {performing_user_id}.")
        return True
