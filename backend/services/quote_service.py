...
class QuoteService:
    ...
    def respond_to_quote(self, quote_id, response_data):
        ...
        try:
            ...
            for item_response in response_data["items"]:
                quote_item = (
                    db.session.query(QuoteItem)
                    .filter_by(id=item_response["item_id"], quote_id=quote_id)
                    .first()
                )
                if quote_item:
    ...
    def respond_and_add_to_cart(self, quote_id, response_data):
        ...
        try:
            ...
            db.session.rollback()
            self.logger.error(
                f"Error in respond_and_add_to_cart for quote {quote_id}: {e}"
            )
            raise
    ...
    def update_quote(quote_id, data):
        ...
        try:
            ...
            if "total_amount" in data:
                quote.total_amount = float(
                    InputSanitizer.sanitize_input(data["total_amount"])
                )
            if "notes" in data:
            ...
            MonitoringService.log_info(f"Updated quote {quote_id}", "QuoteService")
            return quote
        ...
        try:
            ...
            if hasattr(quote, "invoice") and quote.invoice:
                raise ValidationException(
                    "Cannot delete quote that has been converted to invoice"
                )
            ...
            MonitoringService.log_info(f"Deleted quote {quote_id}", "QuoteService")
            return True
    ...
    def approve_quote(quote_id):
        ...
        try:
            ...
            MonitoringService.log_info(f"Approved quote {quote_id}", "QuoteService")
            return quote
    ...
    def reject_quote(quote_id, reason=None):
        ...
        try:
            ...
            MonitoringService.log_info(f"Rejected quote {quote_id}", "QuoteService")
            return quote
    ...
    def get_quotes_by_account(b2b_account_id):
        ...
        try:
            ...
            quotes = (
                Quote.query.filter_by(b2b_account_id=b2b_account_id)
            ...
            MonitoringService.log_error(
                f"Error getting quotes for account {b2b_account_id}: {str(e)}"
            )
