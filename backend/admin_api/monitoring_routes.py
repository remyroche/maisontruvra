...
def get_system_health():
    ...
    try:
        ...
        return jsonify(
            status="success",
            data={"is_healthy": is_healthy, "components": health_status},
        ), 200
    except Exception:
        # Log error e
        return jsonify(
            status="error", message="An error occurred while checking system health."
        ), 500
...
def get_latest_errors():
    ...
    try:
        ...
        return jsonify(
            status="error",
            message="An internal error occurred while fetching error logs.",
        ), 500
...
def get_error_logs():
    ...
    try:
        ...
        logs_pagination = MonitoringService.get_error_logs_paginated(
...
def get_recent_errors():
    ...
    errors = {
        "message": "Fetching recent errors requires a dedicated logging/monitoring system."
    }
