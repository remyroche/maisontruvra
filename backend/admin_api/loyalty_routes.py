...
@api_resource_handler(
    model=LoyaltyTier, allow_hard_delete=False
)  # Hard delete is disallowed for safety
...
def get_loyalty_settings():
    ...
    try:
        ...
        # Log error e
        return jsonify(
            status="error", message="Failed to retrieve loyalty settings."
        ), 500
...
def update_loyalty_settings():
    ...
    try:
        ...
        # Log error e
        return jsonify(
            status="error", message="Failed to update loyalty settings."
        ), 500
...
def adjust_user_points(user_id):
    ...
    if not data or "points" not in data or "reason" not in data:
        return jsonify(
            status="error",
            message="'points' (integer) and 'reason' (string) are required.",
        ), 400
