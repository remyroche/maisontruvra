    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if object_id_key not in kwargs:
                abort(
                    500,
                    description=f"Could not find ID key for model {model.__name__} in route.",
                )
...
def api_resource_handler(
    model,
    request_schema=None,
    response_schema=None,
    ownership_exempt_roles=None,
    eager_loads=None,
    log_action=True,
    cache_timeout=3600,
    ...
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            lookup_value = kwargs.get(f"{model.__name__.lower()}_id") or kwargs.get(
                lookup_field
            )
            ...
            if request.method == "DELETE":
                action = "delete"
            elif request.method in ["POST", "PUT"] and request.path.endswith(
                "/restore"
            ):
                action = "restore"
            ...
            try:
                ...
                if lookup_value is not None:
                    ...
                    target_object = query.filter(
                        getattr(model, lookup_field) == lookup_value
                    ).first()

                    if target_object is None:
                        raise NotFoundException(
                            f"{model.__name__} with {lookup_field} {lookup_value} not found."
                        )
                ...
                if ownership_exempt_roles is not None and target_object:
                    ...
                    if not user_is_exempt:
                        if (
                            not hasattr(target_object, ownership_field)
                            or getattr(target_object, ownership_field)
                            != current_user_id
                        ):
                ...
                if action == "delete":
                    ...
                    if is_hard_delete and not allow_hard_delete:
                        raise AuthorizationException(
                            "Hard delete is not permitted for this resource."
                        )
                ...
                if action == "restore":
                    if not hasattr(model, "is_deleted"):
                        raise ServiceException(
                            "This resource does not support restoration.", 400
                        )
                ...
                if request.method in ["POST", "PUT"]:
                    if not request_schema:
                        raise ServiceException(
                            "Server configuration error: missing request schema.", 500
                        )
                    is_partial = request.method == "PUT"
                    g.validated_data = request_schema(partial=is_partial).load(
                        request.get_json()
                    )
                ...
                if log_action:
                    ...
                    target_id = (
                        getattr(result_object, "id", None)
                        if result_object
                        else getattr(g, "target_object", None)
                    )
                    AuditLogService.log_action(
                        action_type,
                        target_id=target_id,
                        details=g.get("validated_data"),
                    )

                final_schema = response_schema or request_schema
                if not final_schema:
                    raise ServiceException(
                        "Server configuration error: missing response schema.", 500
                    )
                ...
                if isinstance(e, ValidationError):
                    raise ServiceException(
                        "Input validation failed", 400, details=e.messages
                    )
                if isinstance(e, SQLAlchemyError):
                    current_app.logger.error(
                        f"Database error in endpoint '{f.__name__}': {str(e)}"
                    )
                ...
                current_app.logger.critical(
                    f"Unexpected Exception in endpoint '{f.__name__}': {str(e)}",
                    exc_info=True,
                )
                raise ServiceException(
                    "An unexpected internal server error occurred.", 500
                ) from e
...
def _execute_and_log_action(func: Callable, *args: Any, **kwargs: Any) -> Any:
    ...
    try:
        ...
        details = (
            f"Endpoint: {request.path}, Method: {request.method}, Status: {status_code}"
        )
        AuditLogService.log_action(
            user_id=user.id, action=func.__name__, details=details, success=is_success
        )
        return response
    except Exception as e:
...
def b2b_user_required(f: Callable) -> Callable:
    ...
    def decorated_function(*args: Any, **kwargs: Any) -> Any:
        ...
        if not claims.get("is_b2b_user", False) or not g.user.is_b2b:
            security_logger.warning(
                f"B2B access denied for user {g.user.email} at {request.path}."
            )
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def decorated_function(*args: Any, **kwargs: Any) -> Any:
            ...
            if not set(permission_names).issubset(user_permissions):
                ...
                return jsonify(
                    {"error": f"Requires permissions: {', '.join(missing_perms)}"}
                ), 403
            return _execute_and_log_action(f, *args, **kwargs)
...
def b2b_admin_required(f: Callable) -> Callable:
    ...
    def decorated_function(*args: Any, **kwargs: Any) -> Any:
        ...
        if not claims.get("is_b2b_user", False) or not g.user.is_b2b:
            security_logger.warning(
                f"B2B Admin access denied for non-B2B user {g.user.email} at {request.path}."
            )
            return jsonify(error="B2B professional account required."), 403
...
def log_admin_action(f: Callable) -> Callable:
    ...
    def decorated_function(*args: Any, **kwargs: Any) -> Any:
        logger.warning(
            "The @log_admin_action decorator is deprecated and can be removed."
        )
        return _execute_and_log_action(f, *args, **kwargs)
