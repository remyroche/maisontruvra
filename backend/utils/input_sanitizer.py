...
class InputSanitizer:
    ...
    def recursive_sanitize(data: Any) -> Any:
        ...
        if isinstance(data, Mapping):  # Use Mapping for dict-like objects
            return {
                key: InputSanitizer.recursive_sanitize(value)
        ...
        if request.is_json and request.json is not None:
            try:
                ...
                security_logger.error(
                    f"Error sanitizing JSON request data: {e}", exc_info=True
                )
...
def sanitize_form_request_data():
    ...
    if request.method in ["POST", "PUT", "PATCH"]:
        ...
        if request.form:
            try:
                sanitized_form_data = InputSanitizer.recursive_sanitize(
                ...
                security_logger.error(
                    f"Error sanitizing form request data: {e}", exc_info=True
                )
                pass  # Continue
...
def sanitize_url_params():
    ...
    if request.args:
        try:
            sanitized_args = InputSanitizer.recursive_sanitize(
            ...
            security_logger.error(
                f"Error sanitizing URL query parameters: {e}", exc_info=True
            )
            pass  # Continue
