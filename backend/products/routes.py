...
def get_products():
    ...
    try:
        ...
        products_pagination = ProductService.get_all_products_paginated(
        ...
        # Log error e
        return jsonify(
            status="error", message="An error occurred while fetching products."
        ), 500
...
def request_stock_notification(product_id):
    ...
    try:
        ...
        logger.exception(
            f"Failed to create stock notification request for product {product_id}"
        )
...
def get_product_categories():
    ...
    try:
        ...
        # Log error e
        return jsonify(
            status="error", message="An error occurred while fetching categories."
        ), 500
...
def get_product_reviews(product_id):
    ...
    try:
        ...
        return jsonify(
            status="success", data=[r.to_dict_for_public() for r in reviews]
        ), 200
    except Exception:
        # Log error e
        return jsonify(
            status="error", message="An error occurred while fetching reviews."
        ), 500
