...
invoice_bp = Blueprint(
    "admin_invoice_routes", __name__, url_prefix="/api/admin/invoices"
)
...
def get_invoices():
    ...
    try:
        ...
        return jsonify(
            status="error",
            message="An internal error occurred while fetching invoices.",
        ), 500
...
def submit_quote_request():
    ...
    try:
        ...
        return jsonify(
            {"message": "Quote request submitted successfully.", "quote_id": quote.id}
        ), 201
