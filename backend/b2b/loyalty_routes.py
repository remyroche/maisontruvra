...
def get_user_referrals():
    ...
    return jsonify([{'referred_user_email': r.referred.email if r.referred else 'N/A', 'status': r.status} for r in referrals])
...
def convert_points():
    ...
    if voucher:
        return jsonify({'voucher_code': voucher.voucher_code, 'discount_amount': voucher.discount_amount})
    return jsonify({'message': 'Conversion failed. Not enough points or invalid amount.'}), 400
...
def redeem_reward():
    ...
    return jsonify({'message': 'Redemption failed. Not enough points or reward not available.'}), 400
