...
def b2b_forgot_password():
    ...
    return jsonify(
        {
            "message": "If a B2B account with that email exists, a password reset link has been sent."
        }
    ), 200
...
def b2b_reset_password():
    ...
    user = AuthService.verify_password_reset_token(
        token, salt="b2b-password-reset-salt"
    )
...
def b2b_register():
    ...
    try:
        ...
        ), 409  # Conflict, e.g., user/company exists
    except Exception:
        # Log error e
        return jsonify(
            status="error",
