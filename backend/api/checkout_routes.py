...
class CheckoutService:
    ...
    def _dispatch_post_order_tasks(self, order, user, points_earned):
        ...
        try:
            ...
            logger.critical(f"CRITICAL: Could not import Celery tasks for post-order processing: {e}", exc_info=True)
        except Exception as e:
            logger.error(f"Failed to dispatch post-order tasks for order {order.id}: {e}", exc_info=True)
