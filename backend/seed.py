...
from backend.models import Category, Collection, Product, ReferralTier, Role, User
    ...
    try:
        ...
        for col_name in collections:
            if not session.query(Collection).filter_by(name=col_name).first():
                session.add(
                    Collection(name=col_name, slug=col_name.lower().replace(" ", "-"))
                )
        ...
        if session.query(Product).count() == 0:
            ...
            products_to_add = [
                Product(
                    ...
                    price=50.00,
                    description="Fresh black summer truffles.",
                    category_id=truffle_cat.id,
                    sku="P001",
                ),
                Product(
                    name="White Alba Truffle",
                    price=200.00,
                    description="Exquisite white alba truffles.",
                    category_id=truffle_cat.id,
                    sku="P002",
                ),
            ]
        ...
        for tier_data in referral_tiers:
            if (
                not session.query(ReferralTier)
                .filter_by(name=tier_data["name"])
                .first()
            ):
