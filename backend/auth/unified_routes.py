...
from backend.schemas import (
    UserRegistrationSchema, LoginSchema, MfaVerificationSchema,
    SetupTotpSchema, MagicLinkRequestSchema, AuthMethodUpdateSchema,
    PasswordResetRequestSchema, PasswordResetConfirmSchema, PasswordChangeSchema
)
...
def login():
    ...
    try:
        ...
        security_logger.warning(f"Failed login attempt for email: {credentials.get('email', 'unknown')} from IP: {request.remote_addr}")
...
def verify_2fa():
    ...
    try:
        ...
        security_logger.warning(f"Failed 2FA attempt for user_id: {mfa_data.get('user_id', 'unknown')} from IP: {request.remote_addr}")
        return jsonify({'error': str(e)}), 401
    except Exception as e:
...
def update_auth_method():
    ...
    try:
        ...
        if success:
            return jsonify({'message': 'Authentication method updated successfully'}), 200
        else:
...
@api_resource_handler(
    model=User,
    request_schema=PasswordChangeSchema,
    log_action=True
)
def change_password():
    """
...
@api_resource_handler(
    model=User,
    request_schema=PasswordResetRequestSchema,
    log_action=True
)
...
@api_resource_handler(
    model=User,
    request_schema=PasswordResetConfirmSchema,
    log_action=True
)
def reset_password():
    """
...
def admin_disable_mfa(user_id):
    ...
    if not current_user or not current_user.is_admin:
        security_logger.warning(f"Non-admin user {current_user_id} attempted to disable MFA for user {user_id}")
...
def admin_trigger_password_reset(user_id):
    ...
    if not current_user or not current_user.is_admin:
        security_logger.warning(f"Non-admin user {current_user_id} attempted to trigger password reset for user {user_id}")
        raise UnauthorizedException('Admin privileges required')
    ...
    # Log the action
    security_logger.info(f"Admin {current_user_id} triggered password reset for user {user_id}")
