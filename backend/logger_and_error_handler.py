...
def handle_validation_error(error):
    ...
    current_app.logger.warning(f"Validation failed for request to {request.path}: {error}")
    response = jsonify({
        "error": {
            "code": 400,
...
def handle_authorization_error(error):
    ...
    current_app.logger.warning(f"Authorization failed for request to {request.path}: {error}")
    response = jsonify({
        "error": {
            "code": 403,
            "message": str(error) or "You do not have permission to perform this action."
        }
    })
...
def handle_marshmallow_validation_error(error):
    ...
    current_app.logger.warning(f"Marshmallow schema validation failed: {error.messages}")
    response = jsonify({
        "error": {
            "code": 400,
...
def handle_generic_exception(e):
    ...
    current_app.logger.critical(f"Unhandled exception at {request.path}: {e}", exc_info=True)
