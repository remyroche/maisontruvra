...
@celery_app.task(
    name="tasks.finalize_order", bind=True, max_retries=3, default_retry_delay=60
)
...
@celery_app.task(
    name="tasks.generate_invoice_pdf", bind=True, max_retries=3, default_retry_delay=60
)
...
def send_order_status_update_task(order_id, new_status):
    ...
    logger.info(
        f"Executing status update email task for order {order_id} to status '{new_status}'."
    )
    notification_service = NotificationService()
...
@celery_app.task(
    name="tasks.process_b2b_quick_order",
    bind=True,
    max_retries=3,
