...
country_delivery_options = db.Table('country_delivery_options',
    db.Column('country_id', db.Integer, db.ForeignKey('countries.id'), primary_key=True),
    db.Column('delivery_option_id', db.Integer, db.ForeignKey('delivery_options.id'), primary_key=True)
)

# Association table for Delivery Options and B2B Tiers
delivery_option_b2b_tiers = db.Table('delivery_option_b2b_tiers',
    db.Column('delivery_option_id', db.Integer, db.ForeignKey('delivery_options.id'), primary_key=True),
    db.Column('b2b_tier_id', db.Integer, db.ForeignKey('b2b_tiers.id'), primary_key=True)
)
