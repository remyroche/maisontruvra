...
class Quote(db.Model):
    ...
    b2b_account_id = db.Column(
        db.Integer, db.ForeignKey("b2b_account.id"), nullable=False
    )
...
class Invoice(db.Model):
    ...
    )  # draft, pending_signature, signed, paid, void
    due_date = db.Column(db.DateTime)
    signature_data = db.Column(db.Text, nullable=True)
    pdf_path = db.Column(db.String(255), nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    b2b_account = db.relationship(
        "B2BAccount", backref=db.backref("invoices", lazy=True)
    )
...
class InvoiceItem(db.Model):
    ...
    invoice = db.relationship(
        "Invoice", backref=db.backref("items", cascade="all, delete-orphan")
    )
