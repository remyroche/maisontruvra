...
class Order(BaseModel, SoftDeleteMixin):
    ...
    order_status = db.Column(
        db.Enum(OrderStatusEnum), nullable=False, default=OrderStatusEnum.PENDING
    )
    total_amount = db.Column(db.Numeric(10, 2), nullable=False)

    # --- Shipping and Tracking ---
    shipping_address_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("addresses.id"), nullable=False
    )
    billing_address_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("addresses.id"), nullable=True
    )
    tracking_number = db.Column(db.String(100))
    creator_ip_address = db.Column(
...
class Invoice(BaseModel, SoftDeleteMixin):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    order_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey("orders.id"), nullable=False, unique=True
    )
