...
class User(BaseModel):
    ...
    loyalty_account = db.relationship(
        "UserLoyalty", back_populates="user", uselist=False
    )
    ...
    b2b_user = relationship(
        "B2BUser", back_populates="user", uselist=False, cascade="all, delete-orphan"
    )
    ...
    referral_tier = db.Column(
        db.Integer, db.ForeignKey("referral_tiers.id"), nullable=True
    )
