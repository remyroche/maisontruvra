...
product_tier_visibility = db.Table(
    "product_tier_visibility",
    db.Column("product_id", db.Integer, db.ForeignKey("products.id"), primary_key=True),
    db.Column(
        "tier_id", db.Integer, db.ForeignKey("loyalty_tiers.id"), primary_key=True
    ),
)
...
class Product(BaseModel, SoftDeleteMixin):
    ...
    )  # e.g., standard, b2b_exclusive, blog
    ...
    reviews = db.relationship(
        "Review", back_populates="product", lazy=True, cascade="all, delete-orphan"
    )
        ...
        data.update(
            {
                ...
                "restricted_to_tier_ids": [
                    tier.id for tier in self.restricted_to_tiers
                ],
...
class ProductVariant(BaseModel, SoftDeleteMixin):
    ...
    stock = db.relationship(
        "Stock", back_populates="variant", uselist=False, cascade="all, delete-orphan"
    )
