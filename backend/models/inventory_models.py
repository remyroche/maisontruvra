...
class Item(db.Model):
    ...
    collection_id = db.Column(
        db.Integer, db.ForeignKey("collections.id"), nullable=True
    )
    ...
    def to_dict(self):
        ...
        final_producer_notes = (
            self.producer_notes
            if self.producer_notes is not None
            else self.product.producer_notes
        )
        final_pairing_suggestions = (
            self.pairing_suggestions
            if self.pairing_suggestions is not None
            else self.product.pairing_suggestions
        )
...
class Inventory(BaseModel):
    ...
    def available_quantity(self):
        ...
        reserved = (
            db.session.query(db.func.sum(InventoryReservation.quantity))
            .filter_by(inventory_id=self.id)
            .scalar()
            or 0
        )
...
class StockMovement(BaseModel, SoftDeleteMixin):
    ...
    serialized_item_id = db.Column(
        pgUUID(as_uuid=True), db.ForeignKey("serialized_items.id"), nullable=True
    )

    quantity_change = db.Column(
        db.Integer, nullable=False
    )  # e.g., +50 for new stock, -1 for a sale
    reason = db.Column(
        db.String(255), nullable=False
    )  # e.g., "Initial Stock", "Order Fulfillment", "Customer Return"
