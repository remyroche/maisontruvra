import uuid
from sqlalchemy.dialects.postgresql import UUID as pgUUID # Use a distinct alias for PostgreSQL UUID
from .. import db
...
class SerializedItem(BaseModel, SoftDeleteMixin):
    ...
    status = db.Column(db.String(50), nullable=False, default='created', index=True) # E.g., 'created', 'in_stock', 'sold'
    order_item_id = db.Column(pgUUID(as_uuid=True), db.ForeignKey('order_items.id'), nullable=True) # Link when sold

    # --- Relationships ---
    product = db.relationship('Product', back_populates='serialized_items')
    passport = db.relationship('ProductPassport', back_populates='serialized_item', uselist=False, cascade="all, delete-orphan")
...
class ProductPassport(BaseModel, SoftDeleteMixin):
    ...
    serialized_item_id = db.Column(pgUUID(as_uuid=True), db.ForeignKey('serialized_items.id'), nullable=False, unique=True)
    
    # --- Relationships ---
    serialized_item = db.relationship('SerializedItem', back_populates='passport')
    entries = db.relationship('PassportEntry', back_populates='passport', cascade="all, delete-orphan", order_by="PassportEntry.timestamp.desc()")
...
class PassportEntry(BaseModel, SoftDeleteMixin):
    ...
    id = db.Column(pgUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    passport_id = db.Column(pgUUID(as_uuid=True), db.ForeignKey('product_passports.id'), nullable=False)
    
    # --- Entry Details ---
    event_type = db.Column(db.String(100), nullable=False) # E.g., 'MANUFACTURED', 'SHIPPED', 'DELIVERED'
