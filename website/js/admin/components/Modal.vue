<!--
 * FILENAME: website/js/admin/components/Modal.vue
 * DESCRIPTION: A reusable modal/dialog component for the Admin Portal.
 *
...
<template>
  <transition name="modal-fade">
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60" @click.self="close">
      <div class="bg-white rounded-lg shadow-2xl w-full max-w-lg mx-auto m-8" role="dialog" aria-modal="true">
        <!-- Modal Header -->
        <header class="flex items-center justify-between p-4 border-b">
          <slot name="header">
            <h2 class="text-xl font-bold text-gray-800">Modal Title</h2>
          </slot>
          <button @click="close" class="text-gray-500 hover:text-gray-800" aria-label="Close modal">&times;</button>
        </header>
        ...
        <!-- Modal Footer -->
        <footer class="flex justify-end p-4 bg-gray-50 border-t">
          <slot name="footer">
