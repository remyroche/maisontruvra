<template>
  <div>
    <h3 class="text-base font-semibold text-gray-900">Restons en contact</h3>
    <p class="mt-2 text-sm text-gray-600">Recevez nos conseils, nos offres et l'actualité de la seconde main.</p>
...
const subscribe = async () => {
  ...
  loading.value = true;
  try {
    // The API client needs to be updated to include this function.
    const response = await apiClient.subscribeB2C(email.value);
    notificationStore.showNotification(response.message || 'Merci pour votre inscription !', 'success');
